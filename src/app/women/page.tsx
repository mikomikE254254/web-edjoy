...
export default function WomenPage() {
  ...
  const { data: womenProducts, isLoading } = useCollection<Product>(productsQuery);
    ...
    <div className="space-y-6">
      ...
      <div className="border-t pt-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 pt-6">
          ...
          {!isLoading && womenProducts && womenProducts.length > 0 ? (
            womenProducts.map(product => (
              <ProductCard key={product.id} product={product} />
            ))
          ) : (
            !isLoading && <p>No products found for this selection.</p>
          )}
        </div>
      </div>
    </div>
